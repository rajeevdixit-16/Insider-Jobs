        ...
        <div className='flex gap-2 mb-6 mt-3'>
          {
            ...
            : <div className='flex gap-2'>
              ...
              <button onClick={()=>setIsEdit(true)} className='text-gray-500 border border-gray-300 px-4 py-2 rounded-lg cursor-pointer'>Edit</button>
              </div>
          }
        </div>
            ...
            {userApplications.map((job,index) => true ?(
              <tr key={index}>
                ...
                <td className='py-2 px-4 border-b'>
                  <span className={`${job.status === 'Pending' ? 'bg-yellow-100 text-yellow-600' : job.status === 'Accepted' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'} px-3 py-1 rounded-full text-sm font-medium`}>
                    {job.status}
                  </span>
                  </td>
              </tr>
